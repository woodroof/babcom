...
Содержимое массива *objects* соответствует содержимому *objects* функции *get\_objects* с параметрами `"get_actions": false`, `"get_template": false`, `"attributes": ["name"]` и `"sort": [{"attribute_code": "name", "type": "asc"}]`.  
...
Если поле *from* отсутствует, возвращаются все объекты списка. Если поле присутствует, возвращаются только изменённые атрибуты и объекты. При отсутствии изменений поле *result* будет содержать **not modified**.  
Если передано поле *attributes*, возвращаются только атрибуты с указанными кодами. Некоторые атрибуты не возвращаются, если не указаны явно.

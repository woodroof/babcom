  ...
  loop
    ...
    if v_checked is not null then
      ...
      v_params :=
        ...
        jsonb_build_object(
          'user_object_id', v_user_object_id,
          'object_id', json.get_integer(in_params, 'object_id'),
          'attribute_id', json.get_integer(in_params, 'attribute_id'));
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
-- Function: attribute_value_fill_functions.fill_value_object_attribute_from_attribute(jsonb)

-- DROP FUNCTION attribute_value_fill_functions.fill_value_object_attribute_from_attribute(jsonb);

CREATE OR REPLACE FUNCTION attribute_value_fill_functions.fill_value_object_attribute_from_attribute(in_params jsonb)
  RETURNS void AS
$BODY$
declare
  v_user_object_id integer := json.get_integer(in_params, 'user_object_id');
  v_value_object_id integer := data.get_object_id(json.get_string(in_params, 'value_object_code'));
  v_object_id integer := json.get_integer(in_params, 'object_id');
...
begin
  ...
  perform data.set_attribute_value_if_changed(v_object_id, v_attribute_id, v_value_object_id, v_attribute_value, v_user_object_id);
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
